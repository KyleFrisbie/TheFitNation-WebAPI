'use strict';

describe('Controller Tests', function() {

    describe('UserExerciseInstanceSet Management Detail Controller', function() {
        var $scope, $rootScope;
        var MockEntity, MockPreviousState, MockUserExerciseInstanceSet, MockUserExerciseInstance, MockExerciseInstanceSet;
        var createController;

        beforeEach(inject(function($injector) {
            $rootScope = $injector.get('$rootScope');
            $scope = $rootScope.$new();
            MockEntity = jasmine.createSpy('MockEntity');
            MockPreviousState = jasmine.createSpy('MockPreviousState');
            MockUserExerciseInstanceSet = jasmine.createSpy('MockUserExerciseInstanceSet');
            MockUserExerciseInstance = jasmine.createSpy('MockUserExerciseInstance');
            MockExerciseInstanceSet = jasmine.createSpy('MockExerciseInstanceSet');
            

            var locals = {
                '$scope': $scope,
                '$rootScope': $rootScope,
                'entity': MockEntity,
                'previousState': MockPreviousState,
                'UserExerciseInstanceSet': MockUserExerciseInstanceSet,
                'UserExerciseInstance': MockUserExerciseInstance,
                'ExerciseInstanceSet': MockExerciseInstanceSet
            };
            createController = function() {
                $injector.get('$controller')("UserExerciseInstanceSetDetailController", locals);
            };
        }));


        describe('Root Scope Listening', function() {
            it('Unregisters root scope listener upon scope destruction', function() {
                var eventType = 'theFitNationApp:userExerciseInstanceSetUpdate';

                createController();
                expect($rootScope.$$listenerCount[eventType]).toEqual(1);

                $scope.$destroy();
                expect($rootScope.$$listenerCount[eventType]).toBeUndefined();
            });
        });
    });

});
