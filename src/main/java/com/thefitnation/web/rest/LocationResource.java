...
import com.thefitnation.service.LocationService;
...
public class LocationResource {
    ...
    @Timed
    public ResponseEntity<List<LocationDTO>> getAllLocations(@ApiParam Pageable pageable, @RequestParam(required = false) String filter)
        throws URISyntaxException {
        if ("gym-is-null".equals(filter)) {
            log.debug("REST request to get all Locations where gym is null");
            return new ResponseEntity<>(locationService.findAllWhereGymIsNull(),
                    HttpStatus.OK);
        }
