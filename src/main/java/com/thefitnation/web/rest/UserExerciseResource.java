...
public class UserExerciseResource {
    ...
    /**
     * SEARCH  /_search/user-exercises?query=:query : search for the userExercise corresponding
     * to the query.
     *
     * @param query the query of the userExercise search 
     * @param pageable the pagination information
     * @return the result of the search
     * @throws URISyntaxException if there is an error to generate the pagination HTTP headers
     */
    @GetMapping("/_search/user-exercises")
    @Timed
    public ResponseEntity<List<UserExercise>> searchUserExercises(@RequestParam String query, @ApiParam Pageable pageable)
        throws URISyntaxException {
        log.debug("REST request to search for a page of UserExercises for query {}", query);
        Page<UserExercise> page = userExerciseService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page, "/api/_search/user-exercises");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
