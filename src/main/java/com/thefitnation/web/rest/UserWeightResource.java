...
public class UserWeightResource {
    ...
    /**
     * SEARCH  /_search/user-weights?query=:query : search for the userWeight corresponding
     * to the query.
     *
     * @param query the query of the userWeight search 
     * @param pageable the pagination information
     * @return the result of the search
     * @throws URISyntaxException if there is an error to generate the pagination HTTP headers
     */
    @GetMapping("/_search/user-weights")
    @Timed
    public ResponseEntity<List<UserWeight>> searchUserWeights(@RequestParam String query, @ApiParam Pageable pageable)
        throws URISyntaxException {
        log.debug("REST request to search for a page of UserWeights for query {}", query);
        Page<UserWeight> page = userWeightService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page, "/api/_search/user-weights");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
